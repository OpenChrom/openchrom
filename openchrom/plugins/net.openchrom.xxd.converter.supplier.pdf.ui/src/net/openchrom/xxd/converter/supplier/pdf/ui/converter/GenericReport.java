 ...
 *******************************************************************************/
package net.openchrom.xxd.converter.supplier.pdf.ui.converter;
...
import net.openchrom.xxd.converter.supplier.pdf.ui.io.GenericPDF;
import net.openchrom.xxd.converter.supplier.pdf.ui.settings.ReportSettingsGeneric;

@SuppressWarnings("rawtypes")
public class GenericReport extends AbstractChromatogramReportGenerator {
	...
	public IProcessingInfo<?> generate(File file, boolean append, List<IChromatogram<? extends IPeak>> chromatograms, IChromatogramReportSettings chromatogramReportSettings, IProgressMonitor monitor) {
		...
		try {
			for(IChromatogram<? extends IPeak> chromatogram : chromatograms) {
				GenericPDF genericPDF = new GenericPDF(settings);
				if(append && file.exists()) {
					File newFile = File.createTempFile("report", ".pdf");
					newFile.deleteOnExit();
					try {
						genericPDF.createPDF(newFile, chromatogram, subMonitor.split(100));
					...
					genericPDF.createPDF(file, chromatogram, subMonitor.split(100));
				}
			}
		} catch(IOException e) {
