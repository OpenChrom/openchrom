...
import net.openchrom.xxd.process.supplier.templates.ui.internal.provider.PeakIntegratorInputValidator;
import net.openchrom.xxd.process.supplier.templates.util.PeakIntegratorListUtil;

public class TemplatePeakIntegrationEditor implements SettingsUIProvider.SettingsUIControl, IExtendedPartUI {
	...
	private AtomicReference<PeakIntegratorListUI> listControl = new AtomicReference<>();
	//
	...
	//
	private IntegratorSettings settings = new IntegratorSettings();
	private IProcessorPreferences<PeakIntegrationSettings> preferences;

	public TemplatePeakIntegrationEditor(Composite parent, IProcessorPreferences<PeakIntegrationSettings> preferences, PeakIntegrationSettings settings) {

		this.preferences = preferences;
		if(settings != null) {
			this.settings.load(settings.getIntegratorSettings());
		}
		//
	...
	public String getSettings() throws IOException {
		...
		if(preferences != null) {
			PeakIntegrationSettings settingz = new PeakIntegrationSettings();
			settingz.setIntegrationSettings(settings.save());
	...
	private void createTableSection(Composite parent) {
		...
		PeakIntegratorListUI peakIntegratorListUI = new PeakIntegratorListUI(parent, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
		Table table = peakIntegratorListUI.getTable();
		...
		table.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				List<?> objects = peakIntegratorListUI.getStructuredSelection().toList();
				//
		...
		//
		peakIntegratorListUI.setEditEnabled(true);
		peakIntegratorListUI.setUpdateListener(new IUpdateListener() {
		...
		//
		Shell shell = peakIntegratorListUI.getTable().getShell();
		ITableSettings tableSettings = peakIntegratorListUI.getTableSettings();
		addDeleteMenuEntry(shell, tableSettings);
		addKeyEventProcessors(shell, tableSettings);
		peakIntegratorListUI.applySettings(tableSettings);
		//
		listControl.set(peakIntegratorListUI);
	}
	...
	private Button createButtonAdd(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				InputDialog dialog = new InputDialog(e.display.getActiveShell(), DIALOG_TITLE, MESSAGE_ADD, PeakIntegratorListUtil.EXAMPLE_SINGLE, new PeakIntegratorInputValidator(settings.keySet()));
				if(IDialogConstants.OK_ID == dialog.open()) {
					String item = dialog.getValue();
					IntegratorSetting setting = settings.extractSettingInstance(item);
					if(setting != null) {
						settings.add(setting);
						setInput();
					}
	...
	private Button createButtonEdit(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(object instanceof IntegratorSetting setting) {
