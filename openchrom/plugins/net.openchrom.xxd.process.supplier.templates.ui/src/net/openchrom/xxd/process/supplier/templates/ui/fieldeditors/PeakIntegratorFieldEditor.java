...
public class PeakIntegratorFieldEditor extends AbstractFieldEditor {
	...
	private PeakIntegratorListUI listUI;
	//
	private static final String FILTER_EXTENSION = "*.txt";
	private static final String FILTER_NAME = "Peak Integrator Template (*.txt)";
	private static final String FILE_NAME = "PeakIntegratorTemplate.txt";
	//
	private static final String CATEGORY = "Peak Integrator";
	...
	private void createTableSection(Composite parent) {
		...
		composite.setLayoutData(gridData);
		//
		listUI = new PeakIntegratorListUI(composite, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
		//
	...
	private Button createButtonAdd(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				InputDialog dialog = new InputDialog(e.display.getActiveShell(), DIALOG_TITLE, MESSAGE_ADD, PeakIntegratorListUtil.EXAMPLE_SINGLE, new PeakIntegratorInputValidator(settings));
	...
	private Button createButtonEdit(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(object instanceof IntegratorSetting) {
					List<IntegratorSetting> settingsEdit = new ArrayList<>();
					settingsEdit.addAll(settings);
					IntegratorSetting setting = (IntegratorSetting)object;
					settingsEdit.remove(setting);
					InputDialog dialog = new InputDialog(e.display.getActiveShell(), DIALOG_TITLE, MESSAGE_EDIT, settings.extractSetting(setting), new PeakIntegratorInputValidator(settingsEdit));
					if(IDialogConstants.OK_ID == dialog.open()) {
						String item = dialog.getValue();
	...
	private void deleteItems(Shell shell) {
		...
		if(MessageDialog.openQuestion(shell, DIALOG_TITLE, MESSAGE_REMOVE)) {
			IStructuredSelection structuredSelection = (IStructuredSelection)listUI.getSelection();
			List<IntegratorSetting> removeElements = new ArrayList<>();
			for(Object object : structuredSelection.toArray()) {
				if(object instanceof IntegratorSetting) {
					removeElements.add((IntegratorSetting)object);
				}
			}
