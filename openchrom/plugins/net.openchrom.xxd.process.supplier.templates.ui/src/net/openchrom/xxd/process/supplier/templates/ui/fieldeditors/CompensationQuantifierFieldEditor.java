...
import net.openchrom.xxd.process.supplier.templates.ui.swt.CompensationQuantifierListUI;
import net.openchrom.xxd.process.supplier.templates.util.CompensationQuantListUtil;

public class CompensationQuantifierFieldEditor extends AbstractFieldEditor {

	private static final int NUMBER_COLUMNS = 2;
	//
	private Composite composite;
	private CompensationSettings settings = new CompensationSettings();
	private CompensationQuantifierListUI listUI;
	//
	private static final String FILTER_EXTENSION = "*.txt";
	private static final String FILTER_NAME = "Compensation Quantifier Template (*.txt)";
	private static final String FILE_NAME = "CompensationQuantifierTemplate.txt";
	//
	private static final String CATEGORY = "Compensation Quantifier";
	private static final String DELETE = "Delete";

	public CompensationQuantifierFieldEditor(String name, String labelText, Composite parent) {
		init(name, labelText);
	...
	private void createTableSection(Composite parent) {
		...
		composite.setLayoutData(gridData);
		//
		listUI = new CompensationQuantifierListUI(composite, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
		//
	...
	private Button createButtonAdd(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				InputDialog dialog = new InputDialog(button.getShell(), DIALOG_TITLE, MESSAGE_ADD, CompensationQuantListUtil.EXAMPLE_SINGLE, new CompensationQuantifierInputValidator(settings.keySet()));
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(object instanceof CompensationSetting) {
					...
					CompensationSetting setting = (CompensationSetting)object;
					keySetEdit.remove(setting.getName());
					InputDialog dialog = new InputDialog(button.getShell(), DIALOG_TITLE, MESSAGE_EDIT, settings.extractSettingString(setting), new CompensationQuantifierInputValidator(keySetEdit));
					if(IDialogConstants.OK_ID == dialog.open()) {
						String item = dialog.getValue();
