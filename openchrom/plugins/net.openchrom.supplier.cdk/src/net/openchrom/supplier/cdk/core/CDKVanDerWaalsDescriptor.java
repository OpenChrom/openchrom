	...
	public double describe(IAtom atom, IMolecule molecule) {
		...
		try {
			...
			val = Double.parseDouble(
					desc.calculate(atom, molecule).getValue().toString());
			
		} catch(IOException e) {
			logger.warn("IOException while trying to describe Van Der Waals radius of molecule instance "+ molecule +
					". This Error is probably not harmful though.\n But check whether your input Atoms make sense!"+
					"   Here is more information about the IOException:\n"+e);
