...
public abstract class AbstractDoubleRange extends VeryAbstractDoubleRange implements RangeDouble {
	...
	/**
	 * This {@code AbstractDoubleRange's} lower limit, which is the smallest possible position that is valid for {@code start}.
	 */
	protected final double limit1;
	// Constructor //
	/**
	 * This {@code AbstractDoubleRange's} upper limit, which is the biggest possible position that is valid for {@code stop}.
	 */
	...
	public boolean includes(final double position) {
		...
		if(position == getStart() || position == getStop())
			return true;
		return ((UtilMath.round(UtilMath.round(position - start, ACCURACY) % interval, ACCURACY) == 0) || (UtilMath.round(UtilMath.round(position - start, ACCURACY) % interval, ACCURACY) == interval)) && position <= stop;
	}
	 ...
	 * @throws RangeException
	 */
	protected abstract AbstractDoubleRange newInstange(double start, double stop, double limit1, double limit2) throws RangeException;
