...
public abstract class ReaderTableAbstract<T> extends AbstractIOIterator<Row<T>> implements ReaderTable<T> {
	...
	protected Row<T> doRead() throws IOException {
		...
		if(isEnableTextWrapping() && line.contains(getTextWrapper()) && !line.endsWith(getTextWrapper())) {
	...
	public synchronized ReaderTableAbstract<T> setFilterRegex(final Collection<? extends Pair<String, String>> regexFilter) {
